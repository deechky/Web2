...
namespace SharingService.Tests
{
    public class HealthEndpointsTests
    {
        ...
        public async Task HealthReady_WhenDatabaseReachable_ReturnsHealthy()
        {
            ...
            Assert.Equal("sqlserver-sharingdb", dbCheck.GetProperty("name").GetString());
        ...
        public async Task HealthReady_WhenDatabaseUnreachable_ReturnsUnhealthyWith503()
        {
            await using var host = await TestHost.StartAsync(new()
            {
                ["ConnectionStrings:SharingDB"] =
                    "Server=localhost,59999;Database=SharingDB;Trusted_Connection=True;TrustServerCertificate=True;Connect Timeout=2",
            });
